...
from serl_launcher.networks.actor_critic_nets import Critic, Policy, ensemblize, ValueCritic
from serl_launcher.networks.lagrange import GeqLagrangeMultiplier
from serl_launcher.networks.mlp import MLP
from serl_launcher.utils.train_utils import _unpack


class CLAgent(flax.struct.PyTreeNode):
    """
    ...
    def forward_log_alpha(
        ...
        """
        Forward pass for log_alpha network.
        Pass grad_params to use non-default parameters (e.g. for gradients).
        """
        if train:
            assert rng is not None, "Must specify rng when training"
    ...
    def critic_loss_fn(self, batch, params: Params, rng: PRNGKey):
        ...
        ## Intervention Loss ##
        intervention_rngs = jax.random.split(rng, 10)
        rng = intervention_rngs[0]
        assert "o1" in batch and "o2" in batch
        assert "constraint_coeff" in self.config and "constraint_eps" in self.config

        log_alpha = self.forward_log_alpha(batch["o1"], batch["o2"], rng=intervention_rngs[1])
        alpha = jnp.clip(jnp.exp(log_alpha), 0, 1e6)

        o1_qf = self.forward_critic(
            batch["o1"],
            self.forward_policy(batch["o1"], rng=intervention_rngs[2]).sample(seed=intervention_rngs[3]),
            rng=intervention_rngs[4],
            grad_params=params
        )
        o2_qf = self.forward_critic(
            batch["o2"],
            self.forward_policy(batch["o2"], rng=intervention_rngs[5]).sample(seed=intervention_rngs[6]),
            rng=intervention_rngs[7],
            grad_params=params
        ) #  (critic_ensemble_size, batch_size)

        qf_diff = jnp.where(
            jnp.abs(o1_qf - self.config["constraint_coeff"] * o2_qf) <= self.config['constraint_eps'] * jnp.maximum(jnp.abs(o1_qf), jnp.abs(o2_qf)),
            0,
            o1_qf - self.config["constraint_coeff"] * o2_qf
        )

        ex_qf = self.forward_critic(batch['o2'], batch['a1'], rng=intervention_rngs[8], grad_params=params)
        pi_qf = self.forward_critic(batch['o2'], batch['a2'], rng=intervention_rngs[9], grad_params=params)

        action_qf_diff = jnp.where(
            jnp.abs(ex_qf - pi_qf) <= self.config['constraint_eps'] * jnp.maximum(jnp.abs(ex_qf), jnp.abs(pi_qf)),
            0,
            ex_qf - pi_qf
        ).squeeze() # N x B

        diffs = jnp.concatenate([qf_diff, action_qf_diff], axis=0).transpose()
        dual_loss = jnp.multiply(alpha, diffs).mean()
        critic_loss += dual_loss

        ## End Intervention Loss ##
    ...
    def log_alpha_loss_fn(self, batch, params: Params, rng: PRNGKey):
        assert "o1" in batch and "o2" in batch
        assert "constraint_coeff" in self.config and "constraint_eps" in self.config
        ...
        log_alpha = self.forward_log_alpha(batch["o1"], batch["o2"], rng=intervention_rngs[1], grad_params=params)
        ...
        ex_qf = self.forward_critic(batch['o2'], batch['a1'], rng=intervention_rngs[8])
        pi_qf = self.forward_critic(batch['o2'], batch['a2'], rng=intervention_rngs[9])
        ...
        diffs = jnp.concatenate([qf_diff, action_qf_diff], axis=0).transpose() # B x 2N

        log_alpha_loss = -jnp.multiply(log_alpha, diffs).mean()
    ...
    def loss_fns(self, batch):
        return {
            ...
            "log_alpha": partial(self.log_alpha_loss_fn, batch),
        }
    ...
    def create(
        ...
        **kwargs,
        ...
        txs = {
            ...
            "log_alpha": make_optimizer(**log_alpha_optimizer_kwargs),
        }
    ...
    def create_pixels(
        ...
        **kwargs,
        ...
        log_alpha_def = nn.Sequential(
            [
                encoders["alpha"],
                MLP(**alpha_network_kwargs, activate_final=True),
                nn.Dense(features = 2 * critic_ensemble_size),
            ],
            name="log_alpha"
        )
